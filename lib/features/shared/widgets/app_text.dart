...
class AppText extends StatelessWidget {
  ...
  String _handleText({
    ...
    String truncateText(String value) {
      ...
      if (textLimit == null || trimmed.length <= textLimit!) {
        return trimmed;
      }

      final String textVal = trimmed.substring(0, textLimit!);
      if (substringForDots != null && substringForDots! > 0) {
        final int end = (textVal.length - substringForDots!).clamp(0, textVal.length).toInt();
        return "${textVal.substring(0, end)}...";
      }
      return textVal;
    }
    ...
    if (text != null) {
      ...
      if (textAr == null) {
        ...
        return truncateText(textAr!);
      }
    } else if (LanguageHelper.isEN(context)) {
      if (textEn == null) {
        return "";
      } else {
        return truncateText(textEn!);
      }
    }
    return "";
  }
